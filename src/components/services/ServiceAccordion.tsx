...
const ServiceAccordion = ({ items }: ServiceAccordionProps) => {
  ...
  const renderItem = (item: AccordionItem, globalIndex: number) => (
    <div 
      key={globalIndex} 
      className={`border-b border-slate-200 transition-all duration-300 ${openIndex === globalIndex ? 'bg-primary' : ''}`}
    >
      <button
        onClick={() => setOpenIndex(openIndex === globalIndex ? null : globalIndex)}
        className="w-full flex items-center justify-between py-6 px-4 text-left group"
      >
        <span className={`text-md md:text-lg font-semibold uppercase tracking-tight transition-colors duration-300 ${openIndex === globalIndex ? 'text-white' : 'text-slate-800 group-hover:text-primary'}`}>
          {item.title}
        </span>
                        ...
                        return (
                          <div key={j} className="flex items-start gap-3 pl-[10px] mt-2">
                            ...
                            <span>
                              {colonIndex !== -1 ? (
                                <>
                                  <span className="font-semibold text-white">{content.substring(0, colonIndex + 1)}</span>
